...
class Password_Protected {
	...
	public function maybe_show_login() {
		...
		// If a password token is passed in the URL (e.g. from a MailChimp newsletter in order to bypass password protection)
		// but the password-protected request isn't included, redirect to the login page with the correct query string parameters
		if(isset($_REQUEST['pwd_token']) && ! isset($_REQUEST['password-protected'])) {
			$redirect_to = add_query_arg( [
				'password-protected' => 'login',
				'pwd_token' => $_REQUEST['pwd_token']
			], home_url() );
		...
		// If a password token is passed in and password-protected too, try to login automatically
		if(isset($_REQUEST['pwd_token']) && isset($_REQUEST['password-protected'])) {
			if($_REQUEST['pwd_token'] == get_option( 'password_protected_password' )) {
				$this->set_auth_cookie();
				$redirect_to = isset( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : '';
				$redirect_to = apply_filters( 'password_protected_login_redirect', $redirect_to );
